...
- **main.cpp のリファクタリング**
  - グローバル変数を機能別に整理（ハードウェア、設定、センサーデータ）
  - 不要な変数（failureCount、data など）を削除または名前変更
  - `loop()` 関数を 3 つの小さな関数に分割：
    - `readSensorData()` - センサーデータ読み込み
    - `processSensorData()` - センサーデータ解析とログ出力
    - `sendAndSleep()` - データ送信とスリープ

- **コード可読性の向上**
  - JSDoc 形式のドキュメンテーションコメントを追加
  - マジックナンバーをコメント付き定数化
  - グローバル変数の説明をセクション化

- **機能の改善**
  - `parseSensorData()` を使用してセンサー解析を統一
  - エラーメッセージを日本語化
  - センサー読み込み回数を `sensor_read_count` に統一

### Technical Details

- メモリ使用量：ほぼ変化なし
  - RAM: 13.5% (44,116 / 327,680 bytes)
  - Flash: 66.8% (875,884 / 1,310,720 bytes)
- コード行数：短縮化（特に loop() が 30 行→20 行に）
